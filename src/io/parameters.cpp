...
void ves::Parameters::read(int argc, const char* argv[])
{
    ...
    systemOptions.add_options()
        ...
        ("system.osmotic_density_inside", po::value<REAL>(), "density of osmotic particles in bulk")
        ...
        ("system.temperature,t", po::value<REAL>(), "temperature")
        ("system.time_max", po::value<std::size_t>()->default_value(std::numeric_limits<std::size_t>::max()), "time step")
        ("system.ljepsilon,k", po::value<REAL>()->default_value(1.0), "kappa")
        ("system.ljsigma,k", po::value<REAL>()->default_value(1.0), "kappa")
        ("system.kappa,k", po::value<REAL>()->default_value(1.0), "kappa")
        ("system.gamma,g", po::value<REAL>()->default_value(11), "gamma angle")

        ("system.sw_position_min", po::value<REAL>()->default_value(0.05), "position stepwidth min (MonteCarlo only)")
        ("system.sw_position_max", po::value<REAL>()->default_value(1.0), "position stepwidth min (MonteCarlo only)")
        ("system.acceptance_position_target", po::value<REAL>()->default_value(0.3), "position stepwidth min (MonteCarlo only)")
        ("system.sw_orientation_min", po::value<REAL>()->default_value(M_PI_4/20), "orientation stepwidth min (MonteCarlo only)")
        ("system.sw_orientation_max", po::value<REAL>()->default_value(M_PI_4), "orientation stepwidth max (MonteCarlo only)")
        ("system.acceptance_orientation_target", po::value<REAL>()->default_value(0.3), "orientation stepwidth max (MonteCarlo only)")
